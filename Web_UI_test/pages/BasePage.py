...
class BasePage:
    ...
    def assert_len(self, locator, x: int):
        ele = self.dr.find_element(*locator)
        assert len(ele.text) > x
    ...
    def element_should_be_disappear(self, locator, timeout=2):
        WebDriverWait(self.dr, timeout).until_not(EC.visibility_of_element_located(locator))

    def change_frame(self, locator):
        iframe = self.dr.find_element(*locator)
